...
internal fun String.insertEmoji(emoji: CustomEmoji, state: EditorStore.State): String {
...
internal fun String.getNewPosition(emoji: CustomEmoji, state: EditorStore.State): Pair<Int, Int> {
    val code = ":${emoji.shortcode}:"
    val limit = state.statusCharactersLimit
...
internal fun String.getNewLength(emoji: CustomEmoji, state: EditorStore.State): Int {
    val code = ":${emoji.shortcode}:"
    val limit = state.statusCharactersLimit
...
internal fun String.insertHint(hint: EditorInputHintItem, state: EditorStore.State): String {
    val markers = setOf(':', '#', '@')
...
internal fun String.getNewPosition(hint: EditorInputHintItem, state: EditorStore.State): Pair<Int, Int> {
    val markers = setOf(':', '#', '@')
    val currentText = state.statusText
    val currentSelection = state.statusTextSelection
    val textToInsert = when (hint) {
...
internal fun String.getNewLength(hint: EditorInputHintItem, state: EditorStore.State): Int {
